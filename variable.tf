...
variable "provider_vsphere_host" {
  description = "vCenter server FQDN or IP - Example: vcenter.hamzuy.lab"
}
...
variable "deploy_vsphere_host" {
  ...
  type        = string
  default     = "192.168.20.3"  # Your ESXi host IP/name
}
...
variable "master_ips" {
  ...
  type        = list(string)
  default     = ["10.20.110.5"]  # List format
}

variable "worker_ips" {
  description = "IP addresses for worker nodes"
  type        = list(string)
  default     = ["10.20.110.6", "10.20.110.7"]  # List format
}
